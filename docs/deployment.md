...
`GET /healthz` returns `200 ok` once the process is up (it does not check
database connectivity itself, since migrations having already run at
startup is the real readiness signal). Docker Compose's healthcheck for
`server` uses this endpoint.
