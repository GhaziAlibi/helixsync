...
// Long backoffs don't need that: the periodic sync alarm (background/
// index.ts) calls ensureConnected() on every tick and resumes any
// still-pending deadline. Only hold the worker for short waits where the
// alarm would add noticeable latency — applies both to freshly scheduled
// backoffs (scheduleReconnect) and to resumed post-restart cooldowns
// (ensureConnected).
 ...
 * reconnect is already scheduled. Concurrent callers share one in-flight
 * connect attempt instead of each opening their own socket (which doubled
 * connections against the server's per-device limit and duplicated every
 * push handler). */
...
export async function ensureConnected(): Promise<void> {
  ...
  if (Date.now() < reconnectBlockedUntil) {
    ...
    // a fresh timer for whatever's left of the deadline — unless so much is
    // left that holding the worker beats the next alarm tick (see
    // SHORT_RECONNECT_HOLD_MS), in which case the alarm resumes it. This
    // does not touch reconnectDelayMs/reconnectBlockedUntil themselves —
    // nothing new is being scheduled, an existing wait is just continuing.
...
/** Starts one connect attempt shared by concurrent callers (see
 * `connectPromise` above). Unexpected throws become warn + backoff rather
 * than unhandled rejections — the expected failure modes already schedule
 * their own reconnect inside `connect()` and return normally, so this catch
...
function scheduleReconnect(): void {
  if (reconnectTimer) return;
  // Snapshot the current delay once: the persisted deadline and the armed
  // timer must agree. (Previously the deadline used the pre-doubled value
  // while the timer armed the post-doubled one, so every wait ran 2x long —
  // first retry at 2s instead of 1s — and a restart mid-wait resumed a
  // shorter deadline than the timer it replaced.)
  const delay = reconnectDelayMs;
  // Persist the deadline first so a restart still honors the backoff even
  // when no timer is armed below. The persisted deadline stays unjittered
  // (it is the canonical "not before" instant every worker instance agrees
  // on); only the in-memory timer below is jittered.
  ...
  // Decorrelated jitter on the armed timer only: devices disconnected by the
  // same server-side event (restart, handshake-limit window) otherwise retry
  // in lockstep and re-trip the IP-keyed handshake limit together on every
  // cycle. [0.5x, 1x) keeps every retry within its nominal backoff window —
  // never later than the unjittered schedule, so worst-case latency is
  // unchanged and restart-resume math (which uses the unjittered deadline
  // above) is unaffected.
  armReconnectTimer(delay * (0.5 + Math.random() * 0.5));
}

/** Whether the push channel is currently usable. Used by sync/engine.ts to
 * skip an empty download poll when a healthy WebSocket already delivers
 * peer changes promptly — the alarm's upload probe still runs every tick,
 * so this only ever delays *download* staleness, bounded by
 * DOWNLOAD_SKIP_WINDOW_MS there. Synchronous on purpose: the engine's
 * skip check needs an answer without awaiting storage. */
