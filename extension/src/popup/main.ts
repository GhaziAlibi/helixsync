...
// Guards the Connect flow against double-submit: registration happens
// server-side *before* the slow Argon2id derivation finishes, so two rapid
// clicks (or a manual click racing the pendingConnect resume below) used to
// mint two distinct device rows for one browser. While true, submit/resume
// paths return early and the submit button stays disabled.
...
// Drafts are written as the user types — debounce so every keystroke doesn't
// hit chrome.storage.local (disk) individually. Trailing-edge: the last
// input always lands ~400ms after typing stops, and submit/save paths call
// saveDraft directly (not through this) so nothing is lost if the popup
// closes mid-debounce.
 ...
 * displays another device's synced content. See README.md "Limitations":
 * remote visits are never replayed into `chrome://history` at all (see
 * history/index.ts), so this list is the only place the real title/url/
 * visitedAt of a synced remote visit is ever shown. */
 ...
 *
 * `initial` is just the first page (see `PENDING_RESTORE_PAGE_SIZE` and
 * `getPendingTabRestoresLimited`): `totalCount` sizes the labels, and the
 * "Show more" path fetches the remainder on demand via `loadRemaining`
 * instead of making every popup open materialize every tracked tab. Already-
 * rendered (or already-restored) rows are filtered out of the late arrival
 * so a restore racing the fetch can't duplicate a row. */
...
async function renderSetup(): Promise<void> {
  ...
  form.addEventListener("submit", async (e) => {
    e.preventDefault();
    if (connectInFlight) return; // double-submit while deriving — see flag docs
    ...
    try {
      ...
      // Fire-and-forget: chrome.permissions.request() below must be reached
      // with no prior `await`, or Chrome rejects it with "must be called
      // during a user gesture" — an awaited chrome.storage.local.set() is
      // enough on its own to consume this click's transient activation.
      // The write still lands in the browser process even if the
      // permission prompt kills this popup right after (see pendingConnect
      // doc comment below).
      void saveDraft<SetupDraft>(SETUP_DRAFT_KEY, { serverUrl, email, password, deviceName, pendingConnect: true });
  ...
  if (draft.pendingConnect && draft.serverUrl && draft.email && draft.password) {
    ...
    if (alreadyGranted) {
      if (connectInFlight) return; // manual submit already running — same guard
      connectInFlight = true;
      try {
...
async function attemptConnect(
  ...
  try {
    // Host permission for the server origin is requested by callers before
    // attemptConnect runs (chrome.permissions.request() needs an unbroken
    // user-gesture chain, which this function's earlier awaits would break).
    ...
    //
    // Fire-and-forget, not awaited: the background handler for this message
    // runs `initializeCaptureForSettings`, which (on first connect, when
    // `initialImportCompletedAt` isn't set yet) performs the full one-time
    // bookmark+history backfill *before* replying. Awaiting it here used to
    // pin this popup on "Deriving encryption key…" for as long as that
    // backfill takes — minutes on a real profile — with no progress
    // indicator, looking exactly like a permanent freeze even though the
    // device record was already saved above. Nothing below depends on the
    // backfill having started, let alone finished.
...
async function renderStatusView(device: DeviceRecord): Promise<void> {
  ...
  // These reads are independent (background status, cached settings, local
  // history view) — run them concurrently instead of serially so one slow
  // source doesn't gate every popup open behind the sum of all latencies.
  const [response, settings, visits] = await Promise.all([
    chrome.runtime.sendMessage({ type: "GET_STATUS" }),
    fetchSettings().catch(() => undefined),
    // Bounded to what renderHistoryList actually shows (slice(0, 5)):
    // the default limit=20 walks 60 rows to display 5 (91.7% waste).
    getSyncedHistoryVisits(5),
  ]);
  ...
  if (settings?.tabRestorePolicy === "ask") {
    // Bounded initial page plus total count in one mapping fetch + one
    // cursor walk; the full list is only materialized if the user actually
    // clicks "Show more".
  ...
  const historyList = renderHistoryList(visits);
...
async function renderSettings(device: DeviceRecord): Promise<void> {
  ...
  const persistSettingsDraft = () => {
    saveDraftDebounced<SettingsDraft>(SETTINGS_DRAFT_KEY, {
