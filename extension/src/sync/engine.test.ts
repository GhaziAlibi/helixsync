...
vi.mock("../storage/db", () => ({
  ...
  markUploadInFlightRecords: vi.fn(async (records: PendingOperationRecord[]) => {
    for (const record of records) {
      const stored = store.get(record.operation.operationId) ?? record;
      stored.state = "UPLOAD_IN_FLIGHT";
      store.set(stored.operation.operationId, stored);
    }
  }),
  ...
  requeueInFlightRecords: vi.fn(async (records: PendingOperationRecord[], incrementAttempts = false) => {
    for (const record of records) {
      const stored = store.get(record.operation.operationId) ?? record;
      stored.state = "LOCAL_QUEUED";
      if (incrementAttempts) stored.attempts += 1;
      store.set(stored.operation.operationId, stored);
    }
  }),
...
const { uploadPending, runSyncCycle, clearSyncBlockedState, MAX_UPLOAD_ATTEMPTS, applySnapshot, downloadAndApply, shouldChainAnotherCycle } = await import("./engine");
const { getDevice, getPendingOperations, markAppliedBatch, getSyncState, putSyncState, getAppliedOperationIds } = await import("../storage/db");
const { downloadChanges } = await import("../api/client");
...
describe("uploadPending backlog signaling and runSyncCycle chaining", () => {
  beforeEach(async () => {
    store = new Map();
    uploadOperationsMock.mockReset();
    vi.mocked(getPendingOperations).mockReset();
    ...
    vi.mocked(getDevice).mockReset();
    vi.mocked(getDevice).mockResolvedValue(undefined);
    stubChromeStorageSession();
    // Reset any cooldown/blocked state leaked by earlier describes so this
    // describe's cycles always reach the server mock.
    await clearSyncBlockedState();
    uploadOperationsMock.mockImplementation(async (ops) => ({
      accepted: ops.map((op) => op.operationId),
      duplicate: [],
  ...
  it("returns false on an empty queue", async () => {
    await expect(uploadPending()).resolves.toBe(false);
  });

  it("returns false when the queue drains within one call", async () => {
    for (let i = 0; i < 10; i++) {
      const record = pendingRecord(`op-${i}`);
      store.set(record.operation.operationId, record);
    }
  ...
  it("returns true when the backlog exceeds one call's batch budget", async () => {
    // 50 full batches plus one extra op: a single uploadPending call drains
    // exactly its 50-batch budget and must report that more remains.
    for (let i = 0; i < 50 * 500 + 1; i++) {
      const record = pendingRecord(`op-${i}`);
      store.set(record.operation.operationId, record);
    }
    await expect(uploadPending()).resolves.toBe(true);
    expect(store.size).toBe(1);
  });

  it("runSyncCycle chains pairs to drain a multi-cycle backlog in one invocation", async () => {
    for (let i = 0; i < 50 * 500 + 100; i++) {
      const record = pendingRecord(`op-${i}`);
      store.set(record.operation.operationId, record);
    }
    await runSyncCycle();
    expect(store.size).toBe(0);
    // 50 batches in the first pair plus 1 batch in the chained pair.
    // (Download short-circuits: the mocked getDevice returns undefined.)
    expect(uploadOperationsMock).toHaveBeenCalledTimes(51);
  });
});
...
describe("downloadAndApply idle-tick write skip (perf audit P3)", () => {
  const device = {
    id: "self" as const,
    deviceId: "dev-1",
    userId: "user-1",
  ...
  beforeEach(() => {
    ...
    vi.mocked(putSyncState).mockReset();
    vi.mocked(getAppliedOperationIds).mockReset();
    vi.mocked(getAppliedOperationIds).mockResolvedValue(new Set());
    vi.mocked(downloadChanges).mockReset();
  });

  it("does not write sync state for an empty page with an unchanged cursor", async () => {
    vi.mocked(getSyncState).mockResolvedValue({ id: "self", cursor: 42, lamportClock: 1, deviceSequence: 1 });
    vi.mocked(downloadChanges).mockResolvedValue({ operations: [], nextCursor: 42, hasMore: false });

    await downloadAndApply();

    expect(downloadChanges).toHaveBeenCalledTimes(1);
    expect(putSyncState).not.toHaveBeenCalled();
  });

  it("still writes sync state when the cursor advances with zero ops", async () => {
    vi.mocked(getSyncState).mockResolvedValue({ id: "self", cursor: 42, lamportClock: 1, deviceSequence: 1 });
    vi.mocked(downloadChanges).mockResolvedValue({ operations: [], nextCursor: 43, hasMore: false });

    await downloadAndApply();

    expect(putSyncState).toHaveBeenCalledTimes(1);
    expect(vi.mocked(putSyncState).mock.calls[0][0]).toMatchObject({ cursor: 43 });
  });
});

describe("shouldChainAnotherCycle (perf audit P3 chain time-budget)", () => {
  it("chains while backlog remains within count and time budgets", () => {
    expect(shouldChainAnotherCycle(true, 0, 0, 1_000)).toBe(true);
    expect(shouldChainAnotherCycle(true, 3, 0, 59_999)).toBe(true);
  });

  it("stops when there is no more backlog", () => {
    expect(shouldChainAnotherCycle(false, 0, 0, 1_000)).toBe(false);
  });

  it("stops at the count budget even with time left", () => {
    expect(shouldChainAnotherCycle(true, 4, 0, 1_000)).toBe(false);
  });

  it("stops at the wall-clock budget even with count left", () => {
    expect(shouldChainAnotherCycle(true, 0, 0, 60_000)).toBe(false);
    expect(shouldChainAnotherCycle(true, 1, 1_000, 61_001)).toBe(false);
  });
});
