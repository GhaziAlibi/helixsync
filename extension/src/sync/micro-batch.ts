 ...
 * flush's async work finishes — not a full guarantee. */
export function flushAllMicroBatchQueues(): void {
  for (const flushNow of pendingFlushes) flushNow();
}
 ...
 * list above so `flushAllMicroBatchQueues` can trigger it externally (e.g.
 * on onSuspend) in addition to its own timer. `flushNow` is the exact same
 * function for both paths — not duplicated — and is idempotent (clears any
 * pending timer and no-ops on an empty queue), so an onSuspend that races
 * with the timer's own natural firing can't double-flush or leave stale
 * `queue`/`timer` state. */
export function createMicroBatchQueue<T>(flush: (items: T[]) => Promise<void>): (item: T) => void {
  ...
  return function enqueue(item: T): void {
    ...
    timer = setTimeout(flushNow, FLUSH_DELAY_MS);
  };
}
