...
use crate::middleware::rate_limit::{enforce, LOGIN_EMAIL_LIMIT, LOGIN_LIMIT, REGISTER_LIMIT};
...
/// Shortest password accepted on register and password change.
...
async fn register(
    ...
    let password_hash = hash_password_async(req.password, &state.argon2_semaphore)
        .await
    ...
    finish_login(&state, jar, headers, user.id, user.email, ip).await
}
...
/// Does not revoke other sessions or devices; those are revoked separately
/// (docs/security.md §1).
async fn change_password(
    ...
    let new_hash = hash_password_async(req.new_password, &state.argon2_semaphore)
        .await
        .map_err(AppError::Internal)?;
    sqlx::query!(
